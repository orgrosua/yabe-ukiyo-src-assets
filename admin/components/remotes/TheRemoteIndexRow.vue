...
const updatedAt = computed(() => {
    return new Date(props.item.updated_at * 1000);
});
...
function doClipboard() {
    const payload = btoa(`${props.item.remote_url}\n${props.item.title}\n${props.item.license_key}`);
...
<template>
    <transition mode="out-in">
        ...
        <tr v-else :class="{ 'active': item.status, 'inactive': !item.status }" class="yabe-ukiyo-license-row">
            ...
            <td class="vertical-align:middle rel w:1/4">
                <div class="flex align-items:center">
                    <router-link :to="{ name: 'remotes.show', params: { id: item.id } }" :class="{
                ...
                <div class="row-actions visible">
                    <router-link :to="{ name: 'remotes.edit', params: { id: item.id } }"> {{ __('Edit', 'yabe-webfont') }} </router-link>
                    |
            ...
            <td class="vertical-align:middle rel w:1/4">
                <div class="flex align-items:center">
                    ...
                    <div class="flex align-items:center ">
                        {{ item.remote_url }}
                        <a :href="item.remote_url" target="_blank" rel="noopener noreferrer">
                            <font-awesome-icon :icon="['fas', 'up-right-from-square']" class="font:11 mx:4 top:-1.5 absolute" />
                        </a>
                    </div>
                </div>
            </td>
            <td class="vertical-align:middle rel w:2/5">
                <div class="flex align-items:center">
                    {{ item.site }}
