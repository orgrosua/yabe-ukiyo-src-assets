...
import TheSiteIndexRow from '../../components/sites/TheSiteIndexRow.vue';
...
function doBulkActions(action) {
    ...
    switch (action) {
        case 'delete':
            if (
                confirm(__(`Are you sure you want to delete the selected site(s)?`, 'yabe-ukiyo'))
            ) {
        ...
        case 'deactivate':
            if (
                confirm(__(`Are you sure you want to deactivate the selected site(s)?`, 'yabe-ukiyo'))
            ) {
                selectedItems.value.forEach(async (id) => {
                    const item = items.value.find((item) => item.id === id);
        ...
        case 'activate':
            if (
                confirm(__(`Are you sure you want to activate the selected site(s)?`, 'yabe-ukiyo'))
            ) {
                selectedItems.value.forEach(async (id) => {
                    const item = items.value.find((item) => item.id === id);
...
function doRefreshItems() {
    busyStore.add('sites.index:fetch-items');
    api
        .request({
            method: 'GET',
            url: `/admin/licenses/sites/${props.licenseId}`,
            params: {
                page: query.page,
                search: query.search,
        ...
        .finally(() => {
            busyStore.remove('sites.index:fetch-items');
            resetBulkSelection();
        });
}
...
function doUpdateStatus(item, status = null) {
    ...
    busyStore.add('sites.index:update-status');
    item.isUpdatingStatus = true;

    api
        .request({
            method: 'PATCH',
            url: `/admin/sites/update-status/${item.id}`,
            data: {
        ...
        .finally(() => {
            item.isUpdatingStatus = false;
            busyStore.remove('sites.index:update-status');
        });
}

function doDelete(item, ask = true) {
    if (ask && !confirm(__(`Are you sure you want to delete this site?`, 'yabe-ukiyo'))) {
        return;
    }

    item.isDeleting = true;
    busyStore.add('sites.index:delete');

    api
        .request({
            // method: 'DELETE', // not working on IIS server without further configuration
            method: 'POST',
            url: `/admin/sites/delete/${item.id}`,
        })
        .then((response) => {
            item.isDeleted = true;
        })
        ...
        .finally(() => {
            item.isDeleting = false;
            busyStore.remove('sites.index:delete');
        });
}
...
<template>
    ...
    <table class="wp-list-table widefat table-auto min-w:100% plugins">
        <thead>
            <tr>
                ...
                <th scope="col" class="w:3/4">
                    {{ __('Site URL', 'yabe-ukiyo') }}
                </th>
                <th scope="col">
                    {{ __('Registered', 'yabe-ukiyo') }}
                </th>
            </tr>
        </thead>

        <tbody v-if="items.length > 0 && !getBusyHasTask('sites.index:fetch-items')">
            <TheSiteIndexRow v-for="item in items" :key="item.id" :item="item" @delete="doDelete(item)" @update-status="doUpdateStatus(item, null)" />
        </tbody>
        <tbody v-else-if="getBusyHasTask('sites.index:fetch-items')">
            <tr v-for="s in skeleton" class="inactive tw-animate-pulse animation:tw-pulse|cubic-bezier(0.4,0,0.6,1)|2s|infinite">
                ...
                <td class="w:3/10">
                    <div class="h:12 bg:slate-60 rounded w:3/4"></div>
                ...
                <td class="w:3/10 align-items:center vertical-align:middle">
        ...
        <tbody v-else>
            <tr>
                <td colspan="5">
                    {{ __(`It looks like you don't have any sites.`, 'yabe-ukiyo') }}
                </td>
            </tr>
        </tbody>
