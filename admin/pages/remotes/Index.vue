...
import TheRemoteIndexRow from '../../components/remotes/TheRemoteIndexRow.vue';
...
function doRefreshItems() {
    busyStore.add('remotes.index:fetch-items');
    api
        .request({
            method: 'GET',
            url: '/admin/remotes/index',
            params: {
                page: query.page,
                search: query.search,
        ...
        .finally(() => {
            busyStore.remove('remotes.index:fetch-items');
            resetBulkSelection();
        });
}
...
function doUpdateStatus(item, status = null) {
    ...
    busyStore.add('remotes.index:update-status');
    item.isUpdatingStatus = true;

    api
        .request({
            method: 'PATCH',
            url: `/admin/remotes/update-status/${item.id}`,
            data: {
        ...
        .finally(() => {
            item.isUpdatingStatus = false;
            busyStore.remove('remotes.index:update-status');
        });
}

function doDelete(item, ask = true) {
    if (ask && !confirm(__(`Are you sure you want to delete this remote?`, 'yabe-ukiyo'))) {
        return;
    }

    item.isDeleting = true;
    busyStore.add('remotes.index:delete');

    api
        .request({
            // method: 'DELETE', // not working on IIS server without further configuration
            method: 'POST',
            url: `/admin/remotes/delete/${item.id}`,
        })
        .then((response) => {
            item.isDeleted = true;
        })
        ...
        .finally(() => {
            item.isDeleting = false;
            busyStore.remove('remotes.index:delete');
        });
}
...
function doBulkActions(action) {
    ...
    switch (action) {
        case 'delete':
            if (
                confirm(__(`Are you sure you want to delete the selected remote(s)?`, 'yabe-ukiyo'))
            ) {
        ...
        case 'deactivate':
            if (
                confirm(__(`Are you sure you want to deactivate the selected remote(s)?`, 'yabe-ukiyo'))
            ) {
                selectedItems.value.forEach(async (id) => {
                    const item = items.value.find((item) => item.id === id);
        ...
        case 'activate':
            if (
                confirm(__(`Are you sure you want to activate the selected remote(s)?`, 'yabe-ukiyo'))
            ) {
                selectedItems.value.forEach(async (id) => {
                    const item = items.value.find((item) => item.id === id);
...
<template>
    <router-link :to="{ name: 'remotes.create' }" class="page-title-action">{{ __('Add New', 'yabe-ukiyo') }}</router-link>
    ...
    <button type="button" :disabled="busyStore.isBusy" @click="doRefreshItems" v-ripple class="button float:right flex! align-items:center gap-x:5">
        ...
        <font-awesome-icon :icon="['far', 'arrows-rotate']" :class="{ 'animation:rotate|1s|infinite': busyStore.isBusy && getBusyHasTask('remotes.index:fetch-items') }" class="font:15" />
    </button>
    ...
    <table class="wp-list-table widefat table-auto min-w:100% plugins">
        <thead>
            <tr>
                ...
                <th scope="col" class="w:1/4">
                    {{ __('Host URL', 'yabe-ukiyo') }}
                </th>
                <th scope="col" class="w:2/5">
                    {{ __('License Key', 'yabe-ukiyo') }}
                </th>
                <th scope="col" class="w:1/5">
                    {{ __('Last Update', 'yabe-ukiyo') }}
                </th>
            </tr>
        </thead>
        <tbody v-if="items.length > 0 && !getBusyHasTask('remotes.index:fetch-items')">
            <TheRemoteIndexRow v-for="item in items" :key="item.id" :item="item" @delete="doDelete(item)" @update-status="doUpdateStatus(item, null)" />
        </tbody>
        <tbody v-else-if="getBusyHasTask('remotes.index:fetch-items')">
            <tr v-for="s in skeleton" class="inactive animation:tw-pulse|cubic-bezier(0.4,0,0.6,1)|2s|infinite">
                ...
                <td class="align-items:center vertical-align:middle w:1/4">
                    <div class="h:12 bg:slate-60 rounded w:1/2"></div>
                </td>
                <td class="align-items:center vertical-align:middle w:2/5">
                    <div class="h:12 bg:slate-60 rounded w:1/2"></div>
                </td>
                <td class="align-items:center vertical-align:middle w:full">
                    <div class="h:12 bg:slate-60 rounded w:1/2"></div>
                </td>
            </tr>
        </tbody>
        <tbody v-else>
            <tr>
                <td colspan="5">
                    {{ __(`It looks like you don't have any remotes.`, 'yabe-ukiyo') }}
                    <router-link :to="{ name: 'remotes.create' }">{{ __('Perhaps you would like to add a new one?', 'yabe-ukiyo') }}</router-link>
                </td>
            </tr>
        </tbody>
